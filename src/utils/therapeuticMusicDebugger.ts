...
export class TherapeuticMusicDebugger {
  ...
  constructor() {
    ...
    this.displayNameMappings = {
      ...
      'painreducingworld': 'Stress & Anxiety Support', // guessing based on context
...
const filterMusicForBucket = (bucket, tracks) => {
  ...
  testClassicalFocusBucket(musicTracks: any[]) {
    console.log('🧪 TESTING CLASSICAL FOCUS BUCKET:');
    
    const possibleIds = ['classicalfocus', 'focus-music', 'NewAgeandWorldFocus', 'Chopin'];
    ...
    possibleIds.forEach(id => {
      ...
      console.log(`ID "${id}": ${matches?.length || 0} tracks`);
      if (matches?.length > 0) {
        console.log(`  Sample titles: ${matches.slice(0, 3).map(t => t.title || t.name).join(', ')}`);
      }
    });
  }
}
