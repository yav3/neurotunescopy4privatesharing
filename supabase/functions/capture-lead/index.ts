...
serve(async (req) => {
  ...
  try {
    ...
    // Guard against oversized payloads stored in the database
    if (conversationLog && JSON.stringify(conversationLog).length > 50_000) {
      return new Response(
        JSON.stringify({ error: 'Conversation log too large' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }
          ...
          <div style="font-family:-apple-system,BlinkMacSystemFont,'SF Pro Display',sans-serif;max-width:600px;margin:0 auto;background:#0a0a0c;color:#fff;padding:40px;border-radius:16px;">
            ...
            <div style="background:rgba(255,255,255,0.04);border-radius:12px;padding:20px;margin-bottom:24px;">
              <table style="width:100%;border-collapse:collapse;">
                <tr><td style="padding:8px 0;color:#888;font-size:14px;">Ticket Number</td><td style="padding:8px 0;color:#fff;font-size:16px;font-weight:500;font-family:monospace;">${ticketNumber}</td></tr>
                ...
                <tr><td style="padding:8px 0;color:#888;font-size:14px;">Status</td><td style="padding:8px 0;color:rgba(6,182,212,0.9);font-size:14px;">Open</td></tr>
              </table>
            </div>
            ${nextStepsHtml}
            ...
            <a href="https://neurotunes.app/support" style="display:inline-block;background:linear-gradient(135deg,#06b6d4,#2563eb);color:#fff;padding:14px 32px;border-radius:8px;text-decoration:none;font-weight:500;font-size:15px;">Visit Support Center</a>
            <p style="font-size:13px;color:#666;margin-top:32px;">Reply to this email or visit our support center if you need immediate assistance.</p>
          </div>
        `,
      })
      ...
      await resend.emails.send({
        ...
        html: `
          <div style="font-family:-apple-system,BlinkMacSystemFont,'SF Pro Display',sans-serif;max-width:600px;margin:0 auto;background:#0a0a0c;color:#fff;padding:40px;border-radius:16px;">
            ...
            <p style="font-size:13px;color:#666;margin-top:32px;">This lead was automatically captured from a support chat conversation on neurotunes.app</p>
          </div>
        `,
      })
